...
function inicio_total(){ // CALLBACK = buscar_bodegas_buque ()
  ...
  if (localStorage.getItem('ROTACION')) {
    ROTACION_buque = localStorage.getItem('ROTACION');
      buscar_bodegas_buque(function (ROTACION_buque){
          ver_bodegas(ROTACION_buque,function(bodega,tier_seleccion){
            obtener_medidas_tier(bodega,tier_seleccion,function (dato){
              if(bodegas_obj && bodegas_obj.length){
...
function nueva_linga(id,cantidad,tipo_linga,ElementPadre){
      ...
      // var class_linga= $(id).attr("class");
        ...
        guardar_linga(ElementPadre,id); // SE GUARDA LA LINGA CREADAS
...
function boton(){
          ...
          element.setAttribute("onclick",'linga_traspuesta(this,event);');
...
function linga_traspuesta(element,event){
    console.log("ea");
}
...
function dibujar_lingas_tier(){
    ...
    if(Linga[i].giro==0){
          ...
          var linga_tier=$("<div id="+Linga[i].id+" class="+Linga[i].clase+"   onmousedown='opciones_lingas(this,event) onclick='linga_traspuesta(this,event)' ;'><p class='number'>"+Linga[i].cantidad+"</p></div>");
    }else{
        var linga_tier=$("<div id="+Linga[i].id+" class="+Linga[i].clase+" onmousedown='opciones_lingas(this,event) onclick='linga_traspuesta(this,event)' ;'><p class='giro1'>"+Linga[i].cantidad+"</p></div>");
    }
    $(linga_tier).css({
      "position":"absolute",
